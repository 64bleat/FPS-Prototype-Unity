...
namespace MPGUI
{
	...
	public class KeyModelSettingsManager : MonoBehaviour
	{
		...
		{ 
			...
			ff.SetRange(0, 10);
			ff.SetFormat(F2);
			ff.SetReference(_keyModel.mouseSensitivity, VALUE, "Mouse Sensitivity");
			ff = Instantiate(ff, _panel);
			ff.SetReference(_keyModel.sprintToggleTime, VALUE, "Sprint Toggle Time");
			ff = Instantiate(ff, _panel);
			ff.SetReference(_keyModel.walkToggleTime, VALUE, "Walk Toggle Time");
			ff = Instantiate(ff, _panel);
			ff.SetReference(_keyModel.crouchToggleTime, VALUE, "Crouch Toggle Time");
