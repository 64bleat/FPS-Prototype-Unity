...
namespace MPCore.AI
{
	public class Idle : StateMachineBehaviour
	{
		...
		public override void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
		{
			...
			// Update Path Cursor
			position = transform.position - transform.up * body.HitBox.height * 0.5f;
		...
		private Vector3 GetMoveDestination(Component _)
		{
			...
			&& weapons.currentWeapon)
			{
				...
				return Navigator.RandomPoint(body.HitBox.height / 2);
		}
		...
		private Vector2 OnMouseMove(float dt)
		{
				...
				&& character)
			{
				...
				sight.focalPoint = Navigator.PathLerp(path, pPosition, 5f) + body.transform.up * body.HitBox.height;
			else
		...
		private void MoveAlongPath()
		{
			...
			if (body.currentState == CharacterBody.MoveState.Grounded)
			{
				...
				if (upAngle < 40
					|| Physics.SphereCast(transform.position, body.HitBox.radius * 0.5f, transform.forward, out _, body.HitBox.radius * 2))
				{
