...
namespace MPCore
{
	...
	public abstract class GenericDropdownField<T> : MonoBehaviour, IClickable
	{
		...
		ReflectionValue<T> _value;
		readonly List<T> _options = new();
		...
		public void AddOption(T option) => _options.Add(option);
		public void AddOptions(IEnumerable<T> options) => _options.AddRange(options);
		public void RemoveOption(T option) => _options.Remove(option);
		...
		/// </remarks>
		protected virtual string Write(T value) => value != null ? value.ToString() : NULL;
		...
		/// </remarks>
		protected virtual Color Colorize(T option, T current) => (current?.Equals(option) ?? option == null) ? Color.white : Color.grey;
		...
		protected virtual void SetValue(T value) => _value.Value = value;
