...
namespace MPWorld
{
    ...
    public class GravityZone : MonoBehaviour
    {
        ...
        private static readonly List<float> offsetBuffer = new List<float>();
        ...
        public static Vector3 GetVolumeGravity(Collider collider, List<GravityZone> zones, out Vector3 zoneVelocity)
        {
            ...
            for (int i = 0; i < zones.Count; i++)
                if (Physics.ComputePenetration(collider, ctransform.position, ctransform.rotation,
                    zones[i].volume, zones[i].transform.position, zones[i].transform.rotation,
            ...
            if (totalOffset > float.Epsilon)
                for (int i = 0; i < zones.Count; i++)
                {
                    gravity += zones[i].ValueAt(ctransform) * offsetBuffer[i] / totalOffset;

                    if (zones[i].volume.attachedRigidbody)
                        zoneVelocity = Vector3.Max(zoneVelocity, zones[i].volume.attachedRigidbody.velocity);
                }
        ...
        private Vector3 ValueAt(Transform t)
        {
            switch (gravityType)
            {
        ...
        private Vector3 ValueAt(Vector3 worldPosition)
        {
            switch (gravityType)
            {
                case GravityType.Spherical:
