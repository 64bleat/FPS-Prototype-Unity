...
namespace MPCore.AI
{
    public class FollowTarget : StateMachineBehaviour
    {
        ...
        private void RequestNewPath()
        {
            ...
            if (lastSeen > 5f)
            {
                estimate = Navigator.RandomPoint(body.cap.height / 2);
            }
            ...
            // Request Path
            Vector3 localPathOffset = new Vector3(0, -body.cap.height * 0.5f, 0);
            ...
            if (ai.path.Count > 0)
            {
                ...
                if (flatEndOffset > completionDistance)
                {
                    ...
                    if (body.currentState == CharacterBody.MoveState.Grounded)
                    {
                        ...
                        if (upAngle < 40
                            || Physics.SphereCast(ai.transform.position, body.cap.radius * 0.5f, ai.transform.forward, out _, body.cap.radius * 2))
                        {
