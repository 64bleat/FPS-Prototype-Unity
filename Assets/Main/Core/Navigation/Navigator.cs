...
namespace MPCore
{
	...
	public static class Navigator
	{
		...
		/// <param name="path">path to be cleared and re-filled on job completion</param>
		/// <param name="height">how high off the ground the path will be</param>
		/// <returns>a handle to the request job</returns>
		public static JobHandle RequestPath(Vector3 startPosition, Vector3 endPosition, List<Vector3> path, float height = 0)
		{
			NavModel navModel = Models.GetModel<NavModel>();
			PathMesh originPath = null;
			float distance = float.MaxValue;

			foreach (PathMesh pm in navModel.activeMeshes)
				if ((pm.transform.position - startPosition).sqrMagnitude < distance)
					originPath = pm;

			return originPath ? originPath.StartPathJob(startPosition, endPosition, path, height) : default;
		}
		...
		public static Vector3 RandomPoint(float height = 0)
		{
			...
			if (navModel.activeMeshes.Count > 0)
			{
				PathMesh pathMesh = navModel.activeMeshes[UnityEngine.Random.Range(0, navModel.activeMeshes.Count)];
				SplayedMesh splayedMesh = navModel.activeMeshes[UnityEngine.Random.Range(0, navModel.activeMeshes.Count)].SplayedMesh;
				int triangle = UnityEngine.Random.Range(0, splayedMesh.triangles.Length);
				triangle -= triangle % 3;
				...
				return pathMesh.transform.TransformPoint(
					splayedMesh.vertices[splayedMesh.triangles[triangle]] * u / uvw
					+ splayedMesh.vertices[splayedMesh.triangles[triangle + 1]] * v / uvw
					+ splayedMesh.vertices[splayedMesh.triangles[triangle + 2]] * w / uvw)
					+ splayedMesh.normals[triangle / 3].normalized * height;
			}
			else
				return Vector3.zero;
		}
